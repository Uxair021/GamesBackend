/** Pure win-evaluation engine for Sizzling 7s — no imports from services/ or any other game,
 * so this can be imported by both engine.ts (real spins) and services/paytableConfig.ts (RTP
 * simulation) without creating a circular dependency. Mirrors the spec's required function
 * breakdown (evaluatePayline/evaluateAllPaylines/calculateScatterWin/calculateSpinWin/
 * triggerFreeGames/calculateFreeSpinWin) so each rule stays independently testable. */
 ...
 * over candidate A once 3 leading Wilds are in play, since PURE_WILD_PAYOUT[3] (2500) exceeds
 * the best possible substituted win (RED_7 250 x 8 = 2000) — matching the spec's own example
 * ("3 WILDS = 2500, NOT 2500x8").
 */
