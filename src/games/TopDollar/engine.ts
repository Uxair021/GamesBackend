...
export interface SpinResult {
   ...
   * sent to the client in full (see models/TopDollarBonus.ts's doc comment) — routes.ts only
...
function buildLineForTier(tierKey: TierKey): [Symbol, Symbol, Symbol] {
  ...
  if (tierKey === "dollarBonus") {
    const line: Symbol[] = [randomFrom(ALL_REAL_SYMBOLS), randomFrom(ALL_REAL_SYMBOLS), randomFrom(ALL_REAL_SYMBOLS)];
    line[DOLLAR_REEL_INDEX] = "DOLLAR";
    return line as [Symbol, Symbol, Symbol];
  }
...
function buildReel(middle: Symbol): ReelResult {
  return { symbols: [randomFrom(ALL_REAL_SYMBOLS), middle, randomFrom(ALL_REAL_SYMBOLS)] };
}
