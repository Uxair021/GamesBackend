...
import { TopDollarBonus } from "../../models/TopDollarBonus";
...
import { topDollarMeta } from "./meta";
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const paytableConfig = await getPaytableConfig(topDollarMeta.id);
    ...
    const bonus = await TopDollarBonus.create({
      userId: user._id,
      betAmount,
 ...
 * never an index the client picked; see models/TopDollarBonus.ts's doc comment for why the
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const bonus = await TopDollarBonus.findOne({ _id: bonusId, userId: req.userId, status: "pending" });
    if (!bonus) {
